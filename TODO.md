...
- [x] Размеры в баре: systray насильно жмёт любую иконку в квадрат 16×16
      (`[image setSize:16,16]`), поэтому несквадратный PNG растягивало —
      батарейка была слишком широкой. Рисуем её по центру квадратного холста
      (`icon.go`). И шрифт заголовка: у systray нет ручки, тянем NSStatusItem
      через KVC из его делегата и ставим 11pt, как в Python (`cocoa.go`).
- [x] Энергосберегающий режим: в Low Power Mode (NSProcessInfo) обновляемся
      реже (15с вместо 5с) и пишем "low power" в статус-строке.
- [x] Иконка снова авто-генерируемый SVG (как в Python), NSImage сам его
      растеризует — убрали `fogleman/gg` и PNG-кодирование. viewBox квадратный,
      т.к. systray всё равно жмёт в 16×16.
- [x] На старте крутится braille-спиннер в баре, пока не приедет первое
      измерение (первый замер делаем через ~1с, а не через полный интервал).
